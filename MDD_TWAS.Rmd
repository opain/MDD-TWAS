...
![MDD TWAS Heatmap Blood](Images/MDD_TWAS.TWAS_Z_heatmap_brain.png)
...
library(knitr)
kable(res, rownames = FALSE, caption='MDD TWAS Results Novelty', digits = 32)
```
...
Lorenza has prepared results files of previous studies for this comparison.
...
our_hits <- fread("/users/k1806347/brc_scratch/Analyses/Lorenza/Clean/TWAS/MDD_TWAS_AllTissues_CLEAN.txt")
our_hits_sign <- fread("/users/k1806347/brc_scratch/Analyses/Lorenza/Clean/TWAS/MDD_TWAS_AllTissues_TWSig_CLEAN.txt")
Gaspar_hits <- fread("/users/k1806347/brc_scratch/Analyses/Lorenza/Clean/Previous_literature/Gaspar\ et\ al.\ hits.csv")
Wray_hits <- fread("/users/k1806347/brc_scratch/Analyses/Lorenza/Clean/Previous_literature/Wray\ et\ al.\ hits.csv")
...
Gaspar <- unique(sign_genes_Gaspar)
length(Gaspar) #25 unique genes
Wray <- unique(sign_genes_Wray)
length(Wray) #17 unique genes
Us <- unique(sign_genes_our)
length(Us) #94 unique genes 

#create a general variable with all sign. genes in any paper
All <- c(Gaspar, Wray, Us)
length(All) #136 features

#keep only unique gene IDs
All_unique <- unique(All)
length(All_unique) #108 genes
...
our_hits_final <- our_hits_filt[(our_hits_filt$ID %in% All_unique), ]  
dim(our_hits_final)[1] #675 observations
...
library(dplyr)
our_hits_final3  <- our_hits_final2 %>% distinct(ID, .keep_all = T)
dim(our_hits_final3)[1] #107 obs.
...
library(dplyr)
Gaspar_hits4  <- Gaspar_hits3 %>% distinct(ID, .keep_all = T)
dim(Gaspar_hits4)[1] #25 gene IDs 
...
table_correct<- full_join(our_hits_correct, Gaspar_correct)
dim(table_correct)[1] # 108

#merge the newly created table with the wray et al findings too
table_correct2 <- full_join(table_correct, Wray_hits_correct)  #dim 108, 8 
dim(table_correct2)[1] # 108
...
table_correct2$Transcriptome_wide_sign_DallAglio <- ifelse(table_correct2$DallAglio_TWAS.Z > 4.83 | table_correct2$DallAglio_TWAS.Z < -4.83, "Sign.", "Not Sign.")
...
write.csv(my_data2, '/users/k1806347/brc_scratch/Analyses/Lorenza/Clean/Previous_literature/comparison_previousTWASs.csv', row.names=F)
my_data2
