...
<details><summary>Show FOCUS results</summary>
```{R, echo=F, eval=T}
res<-read.csv('/users/k1806347/brc_scratch/Analyses/Lorenza/Clean/FOCUS/MDD_TWAS_FOCUS_results.csv')

res<-res[order(res$chrom,res$tx_start),]
res$region<-factor(res$region, levels=unique(res$region))
res<-res[order(res$region,rev(res$pip)),]

res<-res[c("chrom","tx_start","tx_stop","mol_name","tissue","twas_z","pip","in_cred_set","region")]
names(res)<-c("CHR","P0","P1","ID","PANEL","TWAS.Z","PIP","In Credible Set","Region")

library(knitr)
kable(res, rownames = FALSE, caption='FOCUS MDD Results', digits = 32)
```
...
# Comparison with previous TWAS literature
...
## Wray et al. (2018) and Gaspar et al. (2019)
...
# Comparison with the observed gene expression TWAS of depression 
## Our finding compared to those from Jansen et al. (2016)
